...
const GIGANTES = {
  ...
  PAGINA_FOTO: {
    "inscripcion": "ninos2", "documentos_carga": "entrenadores",
    "mi_estado": "m10m12", "actividades": "tercer", "bingos": "fogata",
    "aportes": "estadio", "sponsors": "playa", "avance": "estadio",
    "galeria_club": "entrenadores", "manual_uso": "m10m12",
    "itinerario": "ninos2", "merchandising": "m10m12",
    "calendario": "tercer", "diploma": "fogata", "guia_familias": "ninos",
    "propuesta_sponsors": "playa", "ruta_familias": "m10m12",
    "ruta_comunidad": "tercer", "ruta_empresas": "playa",
    "album": "ninos2", "album_visual": "entrenadores", "default": "ninos2"
  }
};
...
function injectTopbar() {
  ...
  bar.id = 'gb-topbar';
  bar.innerHTML = `<a class="gb-brand" href="${B}index.html"><img src="${GIGANTES,��GO}" alt="Gigantes" onerror="this.src='https://placehold.co/42/f36b21/fff?text=G'"><div><small>Gigantes de Azapa</small><span>Salta 2026</span></div></a><button class="gb-btn" onclick="document.getElementById('gb-drawer').classList.toggle('open')">☰</button>`;
  ...
  nav.innerHTML = `<div class="gb-dh"><strong>Menú</strong><button class="gb-x" onclick="document.getElementById('gb-drawer').classList.remove('open')">❗</button></div>
    ...
    <a href="${B}merchandising.html">👕 Merch</a>
    <a href="${B}bingos.html">🎰 Bingos</a>
    <a href="${B}sponsors.html">🏢 Sponsors</a>
    <a href="${B}avance.html">📊 H]ance</a>
    <a href="${B}galeria_club.html">📘 Galería</a>
    <a href="${B}manual_uso.html">📋 Manual</a>
    <a href="${B}index.html">🏠 Portada</a>`;
  document.body.appendChild(nav);
  document.addEventListener('click', e => {
    const d = document.getElementById('gb-drawer');
    if (d && !d.contains(e.target) && !e.target.classList.contains('gb-btn')) d.classList.remove('open');
  });
}
...
async function sendToGigantes(data){
  ...
  return new Promise(resolve=>{
    const cb='gCb_'+Date.now();const s=document.createElement('script');const u=new URL(url);
    Object.entries(data).forEach(([k,v])=>u.searchParams.set(k,String(v)));u.searchParams.set('callback',cb);
